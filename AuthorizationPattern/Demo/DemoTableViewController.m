...
//    cameraAuthorizationItem.viewControllerForAlert = self;
    ...
    /// 通知
    UserNotificationsAuthorizationItem *notifyAuthorization = [[UserNotificationsAuthorizationItem alloc] init];
    notifyAuthorization.viewControllerForAlert = self;
    notifyAuthorization.resultCallback = ^(AuthorizationItem *authorizationItem, BOOL authorized) {
        NSLog(@"%@%@", authorizationItem.authorizationName, authorized ? @"授权成功" : @"未授权");
        if (!authorized) return;
        // 后续操作
    };
    [authorizationItems addObject:notifyAuthorization];
